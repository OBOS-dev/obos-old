...
namespace obos
{
	namespace process
	{
		namespace loader
		{
			...
			uint32_t LoadElfFile(const byte* startAddress, size_t size, uintptr_t& entry, uintptr_t& baseAddress, memory::VirtualAllocator& allocator, bool lazyLoad = false);
			uint32_t CheckElfFile(const byte* startAddress, size_t size, bool setLastError = false);
		}
	}
}
